...
func CreateDatabase() {
	...
	_, err := Database.Exec("CREATE TABLE IF NOT EXISTS agent_beliefs (agent_id INTEGER REFERENCES agents(id) ON DELETE CASCADE, belief_id INTEGER REFERENCES beliefs(id) ON DELETE CASCADE, PRIMARY KEY (agent_id, belief_id))")
